...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tv_cmora = findViewById<TextView>(R.id.tv_comra)

        btn_mora.setOnClickListener{
            //若EditText字數小於，則不能開始餐拳
            if(ed_name.length() < 1){
                tv_text.text = "請施入姓名再開始喔喔喔喔"
                return@setOnClickListener  //這啥?! A:label
            }
            //取出EditText文字作為玩家姓名並用變數儲存
            val playerName = ed_name.text
            //亂數產生0-1(不含1)的小數，槍其*30-2後，取整數輸出做為電腦出拳
